...
- ETH news links from GDELT with Cointelegraph RSS fallback data.
...
- Vitalik / Trump crypto-related news is fetched from Google News RSS. The backend filters article titles so the panel only shows items that mention both the person and crypto-related keywords, and returns an explicit empty state without fake data when nothing matches.
