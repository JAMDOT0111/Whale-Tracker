...
- [x] 抓取最新 origin/main 並確認目前 PR 衝突
- [x] 在獨立 worktree 將 origin/main 合併進 feat/crypto-figure-news-feed
- [x] 解決 IMPLEMENTATION.md、Todo.md 與 frontend/src/App.tsx 的衝突
- [x] 在已解衝突的 PR 分支執行後端/前端驗證
- [x] 將更新後的 feat/crypto-figure-news-feed 分支推送到 GitHub

---

## 本次任務：補回 issue-10 分析圖表功能（待補）

### 步驟 1 - 任務盤點與規劃
- [x] 依對話紀錄盤點缺少功能與檔案
- [x] 將補回內容拆分為小步驟寫入 Todo.md

### 步驟 2 - App.tsx 圖表資料與區塊
- [x] 補回交易時間軸（Timeline）資料聚合與 UI 區塊
- [x] 補回交易金額分布圖（Pie）資料聚合與 UI 區塊
- [x] 補回分類中文映射與占比格式化函式
- [x] 補回小比例切片標籤/引線隱藏邏輯（避免重疊）
- [x] 修正時間軸改用實際時間排序與含年份標籤（避免 06-22 誤讀）

### 步驟 3 - SankeyChart.tsx 穩定性與視覺
- [x] 補回中心地址 In/Out 拆分（避免循環渲染失敗）
- [x] 補回中心節點中文文案（中心流入/中心流出）
- [x] 補回 fallback 顯示邏輯（渲染失敗不白屏）
- [x] 補回線寬上限與視覺微調

### 步驟 4 - GraphView.tsx 圖例與標題
- [x] 補回地址關係圖標題調整
- [x] 補回圖例顯示位置調整
- [x] 移除「已標記」圖例項目

### 步驟 5 - 驗證與收斂
- [x] 前端 build 驗證（docker compose build frontend）
- [ ] 功能冒煙測試（地址掃描、Timeline、Sankey、金額分布圖）
- [x] 更新 Todo.md 完成狀態

### 步驟 6 - 地址詳情版面與關聯圖分類清單
- [x] 修正交易歷史卡片下方空白（避免與關聯圖同高撐開）
- [x] 地址關聯圖圖例改為可點擊分類切換
- [x] 顯示所選分類的地址列表（中心/一般/合約/交易所/跨鏈橋）
- [x] 前端 build 驗證並更新 Todo.md 狀態

### 步驟 7 - 地址關聯圖列表化與複製體驗
- [x] 移除地址關聯圖球形發散視覺（保留分類列表）
- [x] 地址列表改為全碼顯示並提供一鍵複製按鈕
- [x] 前端 build 驗證並更新 Todo.md 狀態

### 步驟 8 - 地址關係列表讀取體感與即時資料
- [x] 區塊標題改為「地址關係列表」；錯誤／重試文案改為「關聯標籤」
- [x] 以 `buildTxCounterpartyGraphNodes` 從交易 from/to 即時產生列表（不需等 graph API）
- [x] 後端 graph 背景載入完成後再覆蓋為含 tag／合約等分類的資料（`graphEnriching`）
- [x] 列表捲動區高度略增（`max-h-52`）
- [x] 前端 build 驗證（docker compose build frontend）
