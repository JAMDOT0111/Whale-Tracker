...
func main() {
	...
	candidateService := service.NewCandidateService(store, etherscanClient)
	candidateService.StartScheduler(ctx)
	approvalService := service.NewApprovalService(etherscanClient)
	h := handler.NewHandler(etherscanClient, graphService, store, priceService, newsService, figureNewsService, alertService, candidateService, approvalService)
	...
	api := r.Group("/api")
	{
		...
		api.GET("/candidates/summary", h.GetCandidateSummary)
		api.GET("/candidates/:address", h.GetCandidate)
		...
		api.POST("/admin/candidates/rebuild", h.RebuildCandidates)
